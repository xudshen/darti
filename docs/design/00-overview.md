...
| 3 | Ch2 运行时 | 字节码如何执行 | Ch1（指令格式） |
| 4 | Ch4 编译器 | Kernel AST 如何变成字节码 | Ch1（目标指令集） |
| 5 | Ch3 互调 | 解释器与宿主 VM 如何互调 | Ch2（运行时 API） |
| 6 | Ch5 泛型 | 实化泛型如何实现 | Ch2（栈帧布局）、Ch4（类型传递） |
| 7 | Ch6 异步 | async/await 如何实现 | Ch2（分发循环） |
| 8 | Ch7 沙箱 | 如何保护宿主应用安全 | Ch2（fuel 机制） |
...
| 依赖关系 | 说明 |
|---------|------|
| Ch4 → Ch1 | 编译器生成 Ch1 定义的指令 |
| Ch4 → Ch2 | 编译器产出被 Ch2 运行时加载执行 |
| Ch3 → Ch2 | Bridge/Proxy 通过运行时 API 操作解释器对象 |
| Ch5 → Ch2 | 泛型类型信息存储在栈帧的 ITA/FTA 槽位中 |
| Ch6 → Ch2 | 异步帧快照和恢复扩展分发循环 |
| Ch7 → Ch2 | 沙箱机制复用运行时的 fuel 和调用深度限制 |
