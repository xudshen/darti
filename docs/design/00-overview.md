...
| 5 | Ch4 互调 | 解释器与宿主 VM 如何互调 | Ch3（运行时 API） |
| 6 | Ch5 编译器 | Kernel AST 如何变成字节码 | Ch1（目标指令集）、Ch2（栈帧布局） |
...
箭头含义：`A ──► B` 表示 A 依赖 B 的定义。完整依赖关系见下表。
...
| Ch4 → Ch2 | Bridge/DarticProxy 使用 DarticObject、引用栈等核心数据结构 |
