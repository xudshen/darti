...
| 方向 | 模块 | 接口 |
|------|------|------|
| 输入来源 | Ch5 编译器 | 编译器输出的 `.darb` 字节码文件是验证器的检查对象 |
...
**不受信侧**：`.darb` 字节码文件。可能来源于热更新下发、第三方插件，内容可能格式错误（编译器 bug、传输损坏）或经过恶意篡改。
...
| 检查项 | 内容 | 违规后果 |
|--------|------|----------|
| 文件头 | magic number = `0x44415242`，版本号 <= 当前版本，CRC32 校验和匹配 | 加载拒绝 |
...
| magic number | 0x44415242 | 字节码文件格式定义 |
| 校验和算法 | CRC32 | 文件头验证 |
